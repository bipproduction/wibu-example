...
export function CalendarPage({ data }: { data: string }) {
    ...
    useEffect(() => {
        const events = eventState.flatMap(event => {
            const freq: Frequency = event.repeat === "yearly" ? RRule.YEARLY :
                event.repeat === "monthly" ? RRule.MONTHLY :
                    event.repeat === "weekly" ? RRule.WEEKLY :
                        event.repeat === "daily" ? RRule.DAILY :
                            event.repeat === "hourly" ? RRule.HOURLY :
                                event.repeat === "minutely" ? RRule.MINUTELY :
                                    RRule.SECONDLY;
            ...
            return rule.all().map(recurrenceDate => ({
                ...
                end: new Date(recurrenceDate.getTime() + (event.end.getTime() - event.start.getTime())),
                repeat: event.repeat,
                count: event.count
            }));
        });
            ...
            <Card>
                <Group>
                    <Stack>
                        ...
                        />
                        <Select
                            data={["yearly", "monthly", "weekly", "daily", "hourly", "minutely", "secondly"]}
                            label={"Repeat"}
                            value={form.repeat}
                            onChange={(value) => setForm({ ...form, repeat: value as Event["repeat"] })}
                        />
                        <NumberInput
                            label={"Count"}
                            value={form.count}
                            onChange={(value) => setForm({ ...form, count: value! as Event["count"] })}
                        />
                        <Group>
